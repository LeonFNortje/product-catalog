package network.rain.productcatalog.web.rest;
...
import network.rain.productcatalog.config.KafkaSseConsumer;
import network.rain.productcatalog.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/rain-product-catalog-kafka")
public class RainProductCatalogKafkaResource {

    private final Logger log = LoggerFactory.getLogger(RainProductCatalogKafkaResource.class);
    ...
    public RainProductCatalogKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
