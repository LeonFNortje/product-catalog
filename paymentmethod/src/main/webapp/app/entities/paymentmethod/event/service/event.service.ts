...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'eventTime' | 'timeOccurred'> & {
...
export class EventService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/events', 'paymentmethod');
  ...
  getEventIdentifier(event: Pick<IEvent, 'id'>): string {
    return event.id;
  }
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...event,
      eventTime: event.eventTime?.toJSON() ?? null,
      timeOccurred: event.timeOccurred?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...restEvent,
      eventTime: restEvent.eventTime ? dayjs(restEvent.eventTime) : undefined,
      timeOccurred: restEvent.timeOccurred ? dayjs(restEvent.timeOccurred) : undefined,
    };
  }
