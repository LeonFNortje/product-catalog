...
import network.rain.account.config.KafkaSseConsumer;
import network.rain.account.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/paymentmethod-kafka")
public class PaymentmethodKafkaResource {

    private final Logger log = LoggerFactory.getLogger(PaymentmethodKafkaResource.class);
    ...
    public PaymentmethodKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
