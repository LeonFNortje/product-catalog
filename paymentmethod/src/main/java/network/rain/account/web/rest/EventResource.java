...
/**
 * REST controller for managing {@link network.rain.account.domain.Event}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EventResource {
    ...
    private static final String ENTITY_NAME = "paymentmethodEvent";
        ...
        Optional<Event> result = eventRepository
            ...
            .map(existingEvent -> {
                if (event.getCorrelationId() != null) {
                    existingEvent.setCorrelationId(event.getCorrelationId());
                }
                if (event.getDescription() != null) {
                    existingEvent.setDescription(event.getDescription());
                }
                if (event.getDomain() != null) {
                    existingEvent.setDomain(event.getDomain());
                }
                ...
                if (event.getHref() != null) {
                    existingEvent.setHref(event.getHref());
                }
                if (event.getPriority() != null) {
                    existingEvent.setPriority(event.getPriority());
                }
                if (event.getTimeOccurred() != null) {
                    existingEvent.setTimeOccurred(event.getTimeOccurred());
                }
